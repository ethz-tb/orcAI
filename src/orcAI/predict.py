...
@click.argument("wav_file_path",
    type=click.Path(exists=True, file_okay=True, dir_okay=False, resolve_path=True, readable=True, path_type=Path))
@click.option(
    "--model",
    "-m",
    "model_path",
...
def predict(wav_file_path,
        ...
        output_file = None,
        spectrogram_parameters_path = files("orcAI.defaults").joinpath("default_spectrogram_parameter.json"),
        verbosity = 1):
    """
    ...
    spectrogram, frequencies, times = spec.create_spectrogram(wav_file_path, spectrogram_parameters)
    if spectrogram.shape[1] != shape['input_shape'][1]:
        msgr.error(f"Frequency dimensions of spectrogram shape ({spectrogram.shape[1]}) "+
    ...
    prediction_length = snippet_length // time_steps_per_output_step # Output time steps per prediction
    ...
    aggregated_predictions = np.zeros((total_time_steps, shape['num_labels']))  # Shape: (3600 * 46, 7)
    overlap_count = np.zeros(total_time_steps)  # To track the number of overlaps per time step

    # Step 4: Overlay predictions
    for i, prediction in enumerate(predictions):
        start = i * (shift // time_steps_per_output_step)  # Start index in aggregated predictions
    ...
    msgr.info(f"time for prediction and preparing annotation file: {time.time()-start_time:.2f}")
    ...
    msgr.info(f"number and type signals eliminated because shorter than {minimal_duration} seconds")
    ...
    df_predicted_labels[["start", "stop", "label"]].to_csv(output_file, sep="\t", index=False)
